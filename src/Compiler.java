    ...
    private ParseTree compileNode(ParseTree parseTree) {
        ...
        switch (currentNode.getType()) {
            ...
            case For -> {
                ...
                int comparison = counter -1;
                // Check the condition and goes either to the code or jump at the end of the For
                System.out.printf("        br i1 %%%d, label %%label%d, label %%label%d\n", comparison, forCode, endFor);

                // Creates the code part of the For
                System.out.printf("    label%d:\n", forCode);
                compileNode(parseTree.getChildren().get(2));

                // Jumps back to condition after executing the code in the for
                System.out.printf("        br label %%label%d\n", forCond);

                // Allows for more code to come after the for
                System.out.printf("    label%d:\n", endFor);
